...
//   const clusterValues = clusterLabels.map((id: string) => clusterProfiles[id]?.members ?? 0);
...
//     const h = hex.replace("#", "");
//     const bigint = parseInt(h.length === 3 ? h.split("").map((c: string) => c + c).join("") : h, 16);
...
//   const clusterChart = {
//     labels: clusterLabels.map((id: string) => `Cluster ${id}`),
//     datasets: [
//       {
//         label: "Members",
//         data: clusterValues,
...
//                   {classLabels.map((lab: string, idx: number) => {
//                     const val = classValues[idx] ?? 0;
//                     const total = classValues.reduce((a: number, b: number) => a + b, 0);
...
//                 </div>
//                 <div className="mt-2 text-sm text-gray-600 dark:text-gray-300 text-center">
//                   {clusterLabels.map((lab: string, idx: number) => {
//                     const val = clusterValues[idx] ?? 0;
//                     const total = clusterValues.reduce((a: number, b: number) => a + b, 0);
//                     const pct = total ? ((val / total) * 100).toFixed(1) : "0.0";
//                     return (
//                       <div key={lab}>
//                         Cluster {lab}: {val} ({pct}%)
